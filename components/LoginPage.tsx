...
const LoginPage: React.FC = () => {
  ...
  const handleEmail = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password || (isAnonymous && !name)) {
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-xl w-full max-w-md border border-slate-100 relative">
        ...
        <div className="text-center mb-8">
           ...
           <h1 className="text-2xl font-bold text-slate-800">
             {isAnonymous ? 'Guest Account' : 'Welcome Back'}
           </h1>
           <p className="text-slate-500 text-sm mt-1">
             {isAnonymous 
               ? 'Link an account to save your progress permanently.' 
        ...
        {isAnonymous ? (
            ...
            <div className="space-y-4">
                 <div className="bg-emerald-50 text-emerald-800 p-4 rounded-xl text-center">
                    ...
                    <p className="text-sm opacity-80">You are securely signed in.</p>
                 </div>
                 <button 
